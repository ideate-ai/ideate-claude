...
4. **Enroll new items in the active phase (v2-phase machinery — BOTH paths)**: after creating the new work items, record their membership in the active phase, mirroring `skills/refine/SKILL.md` §7i-1. Read the active phase via `ideate_artifact_query({type: "phase", filters: {status: "active"}})` and hold it as `{current_phase}`. If no active phase is returned (a project without phase tracking), skip this enrollment and note it in the refinement journal entry — there is no phase artifact to record membership in. Otherwise, merge the new WI designations into `{current_phase}.work_items` **append-only** (never overwrite or drop existing references), and write it back via `ideate_write_artifact({type: "phase", id: current_phase.id, content: {merged object}})`. This holds for the v3 board path AND the v2 fallback — phases stay v2 and `work_items` records WI designations for board and v2 items alike, and it is the field the P-47 phase-close gate enumerates as authoritative. **Without this write, autopilot-created items never enter the phase's membership record and silently escape the review gate** (a purely-autopilot project's `work_items` would stay empty). Append-only is required: a phase write that drops existing `work_items` members is refused by the server backstop (WI-331) and would corrupt board-item phase membership.
...
For each work item in `horizon.next`: if it is a **board item** (in `{board_items}` / carries `spec_format: ideate/wi-v1`), do NOT call `ideate_update_work_items` for it — a board item has no v2 artifact record, so that call has nothing to update and would error (it operates on the v2 store only). A board item's lifecycle status lives on the board (`work_get`/`work_list`); its phase membership is recorded when the phase artifact's `work_items` array is written (see the active-phase enrollment step in the normal refinement loop — Instructions step 4), which is v2-phase machinery that stays v2 regardless of the board cutover. For a **v2 item**, call `ideate_update_work_items({updates: [{id: "{work_item_id}", status: "pending", phase: "active"}]})` to promote it from horizon to active scope.
...
- Phase membership — new WI designations enrolled into the active phase's `work_items` (append-only) via `ideate_write_artifact({type: "phase"})` (Instructions step 4; the P-47 gate's authoritative input for both board and v2 items)
