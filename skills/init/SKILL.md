...
- The starting work item number for that module's range — call `ideate_get_next_id({type: "work_item"})` to get the next available number, then allocate ranges to avoid collisions. **Board-aware numbering (v3)**: `ideate_get_next_id` sees only v2 artifacts; if the v3 work-state tools (`work_list`, …) are present in the session (mechanical tool presence — GP-24), also call `work_list` and take the maximum WI number across the artifact index and any board items carrying `spec_format: ideate/wi-v1` before allocating ranges (rare for init, but an init run on a project with existing board state must not reissue board WI numbers). If the work-state tools are absent but `.ideate-work/` exists on disk at the project root, warn loudly before allocating (P-45): "WARNING: this project has board state (.ideate-work/ exists) but the v3 tools are unavailable — likely a missing build (run `pnpm install && pnpm run build` in the plugin). Allocated WI numbers may COLLIDE with board items."
